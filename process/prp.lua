...
-- The top five proposals are active at any given time, so long as each of them exceeds a given threshold of GARD staked.
...
-- Scheduler handler to prevent errors
Handlers.add('scheduler', Handlers.utils.hasMatchingTag('Action', 'Scheduler'), function(msg)
    ao.send({
        Target = msg.From,
        Data = 'Scheduler is active and running.'
    })
end)
